...
watchEffect(() => {
    if (isSuccess.value) {
        notificationIconClass.value = 'w-6 h-6 rounded-full bg-success/20 flex items-center justify-center text-success mr-3 mt-0.5';
        notificationIcon.value = 'fa fa-check';
    } else {
        notificationIconClass.value = 'w-6 h-6 rounded-full bg-error/20 flex items-center justify-center text-error mr-3 mt-0.5';
...
<template>
    ...
    <div id="notification"
        ...
        <div class="flex items-start">
            ...
            <div>
                <h4 id="notificationTitle" class="font-medium text-neutral-800">{{ notificationTitle }}</h4>
                <p id="notificationMessage" class="text-neutral-600 text-sm mt-1">{{ notificationMessage }}</p>
            </div>
