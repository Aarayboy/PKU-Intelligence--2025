...
const upToServer = async (data) => {
  const response = await api.cloud({
    userId: data.userId,
    xuehao: data.xuehao,
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div>
          <label
            ...
            >学号输入 <span class="text-danger">*</span></label
          >
          <input
            type="text"
            id="xuehao"
            v-model="formData.xuehao"
            placeholder="输入学号..."
            required
        ...
        <div>
          <label
            for="password"
            class="block text-sm font-medium text-neutral-700 mb-1"
            >密码输入 <span class="text-danger">*</span></label
          >
          <input
            type="password"
            id="password"
        ...
        <div v-show="choseCourse">
          <label
            for="course"
            class="block text-sm font-medium text-neutral-700 mb-1"
            >课程选择 <span class="text-danger">*</span></label
          >
          <select
            id="course"
            v-model="formData.course"
            :required="choseCourse"
            class="w-full px-4 py-3 rounded-xl border border-neutral-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-custom text-lg"
          >
            <option value="" disabled>请选择课程...</option>
            <option
              v-for="course in cloudCourses"
              :key="course.id"
              :value="course.id"
            >
              {{ course.name }}
