...
export async function uploadNote(payload = {}) {
  ...
  const filesArr = Array.isArray(rawFiles)
    ? rawFiles
    : rawFiles
      ? [rawFiles]
      : [];
...
export function getNoteFileUrl({
  userId,
  lessonName,
  noteName,
  filename,
  ...
  if (!userId || !lessonName || !noteName || !filename) {
    throw new Error(
      "getNoteFileUrl 需要 userId, lessonName, noteName, filename",
    );
  }
  const base = BASE || "";
...
export async function downloadNoteFile({
  ...
  if (!userId || !lessonName || !noteName || !filename) {
    throw new Error(
      "downloadNoteFile 需要 userId, lessonName, noteName, filename",
    );
  }
  const qs = `?userId=${encodeURIComponent(String(userId))}&lessonName=${encodeURIComponent(String(lessonName))}&noteName=${encodeURIComponent(String(noteName))}&filename=${encodeURIComponent(String(filename))}`;
