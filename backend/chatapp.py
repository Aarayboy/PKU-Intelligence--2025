...
async def ask_question(req: ChatRequest):
    ...
    if req.session_id not in sessions:
        raise HTTPException(status_code=400, detail="会话不存在，请先上传 PDF 文件。")
    
    session = sessions[req.session_id]

    # 获取对应会话的 PDF 内容
    if req.session_id not in pdf_contents:
        raise HTTPException(status_code=404, detail="未找到与该会话相关的 PDF 文件。请先上传 PDF 文件。")
    
    pdf_text = pdf_contents[req.session_id]
    print("PDF Text Length:", len(pdf_text))
    print("User Question:", req.message)
    try:
        # 使用 PDF 文本和用户问题进行交互
